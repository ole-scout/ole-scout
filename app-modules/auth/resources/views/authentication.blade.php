<div>
    <form wire:submit="login" class="flex flex-col max-w-xl gap-4 p-8 mx-auto bg-gray-100 rounded-lg shadow mt-80 dark:bg-gray-900">
        <x-ui::field :id="$this->__id . '-email'" type="text" name="loginForm.email" wire:model.live.debounce.200ms="loginForm.email">
            <x-slot:label>{{ __('E-Mail') }}</x-slot:label>
            <x-slot:hint>{{ __('*Pflichtfeld') }}</x-slot:hint>
        </x-ui::field>
        <x-ui::field :id="$this->__id . '-password'" type="password" name="loginForm.password" wire:model="loginForm.password">
